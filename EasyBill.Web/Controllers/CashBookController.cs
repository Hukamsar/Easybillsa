...
namespace EasyBill.UI.Controllers
{
    public class CashBookController : Controller
    {
        ...
        public async Task<IActionResult> Index(DateTime? fromDate, DateTime? toDate)
        {
            ...
            var returnUrl = $"/CashBook/Index?fromDate={fromDate.Value:yyyy-MM-dd}&toDate={toDate.Value:yyyy-MM-dd}";
            var escapedReturnUrl = Uri.EscapeDataString(returnUrl);

            // Get Cash Payment Modes
            var paymentModes = await _modeOfPaymentRepository.GetAll();
            var cashPaymentModeIds = paymentModes
                .Where(x => x.PaymentType == AOne.Utility.Enums.modeofpayment.Cash)
                .Select(x => x.Id)
                .ToHashSet();

            // 1. Sales Cash Transactions
            ...
            var salesTransactions = sales
                ...
                .SelectMany(s => s.SalsePaymentDetails
                    .Where(pd => cashPaymentModeIds.Contains(pd.PaymentModeId) && pd.Amount > 0)
                    .Select(pd => new CashBookDetailVM
                    {
            ...
            // 3. PaymentVoucher Cash Transactions
            ...
            var paymentVoucherTransactions = paymentVouchers
                .Where(pv => pv.PaymentModeId.HasValue && cashPaymentModeIds.Contains(pv.PaymentModeId.Value) && (pv.NetAmount > 0 || pv.Amount > 0))
                .Select(pv => {
            ...
            // 4. ReceiveVoucher Cash Transactions
            ...
            // 5. Contra Cash Transactions
            var contras = await _unitOfWork.GetRepository<Contra>()
                .Query()
                .Include(c => c.Bank)
                .ToListAsync();

            var contraCashTransactions = contras
                .Where(c => c.Amount > 0)
                .Select(c => {
                    decimal debit = 0;
                    decimal credit = 0;
                    ...
                    {
                        credit = c.Amount;
                        particular = c.Bank != null ? $"Deposit to {c.Bank.BankName}" : "Deposit to Bank";
                    }
                    else if (c.Category == AOne.Utility.Enums.ContraCategory.Withdraw)
                    {
                        debit = c.Amount;
                        particular = c.Bank != null ? $"Withdraw from {c.Bank.BankName}" : "Withdraw from Bank";
                    }
        ...
        [HttpGet]
        public async Task<IActionResult> ExportCashBookExcel(DateTime? fromDate, DateTime? toDate)
        {
            ...
            // Report Name (Exactly "Cash Book", no "Report" suffix)
            ws.Cell(2, 1).Value = "Cash Book";
            ...
            wb.SaveAs(stream);
            return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "CashBook.xlsx");
        }

        [HttpGet]
        public async Task<IActionResult> ExportCashBookPdf(DateTime? fromDate, DateTime? toDate)
        {
            var tenantId = User.FindFirst("TenantId")?.Value;
            var tenant = await _unitOfWork.GetRepository<Tenant>().GetAll().FirstOrDefaultAsync(t => t.Id == tenantId);
            string companyName = tenant?.Name ?? "Company";
            ...
            // Report Name (Exactly "Cash Book", no "Report" suffix)
            document.Add(new Paragraph("Cash Book")
                .SetFont(bold)
                .SetFontSize(12)
                .SetTextAlignment(TextAlignment.CENTER));
            ...
            return File(stream.ToArray(), "application/pdf", "CashBook.pdf");
        }
    }
}
